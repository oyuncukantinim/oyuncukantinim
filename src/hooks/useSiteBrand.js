...
const DEFAULT_BRAND = {
  ...
  shopierUserDescription: 'Kart ile bakiye yükle',
  shopierEnabled: false,
  shopierCommissionType: 'none',
  shopierCommissionValue: 0,
  usernameMinLength: 3,
  ...
  _fetchPromise = getSiteSettings()
    .then((response) => {
      ...
      _cachedBrand = {
        ...
        shopierUserDescription: data?.shopier_user_description || DEFAULT_BRAND.shopierUserDescription,
        shopierEnabled: data?.shopier_enabled === 1 || data?.shopier_enabled === '1' || data?.shopier_enabled === true,
        shopierCommissionType: data?.shopier_commission_type || DEFAULT_BRAND.shopierCommissionType,
        shopierCommissionValue: Number(data?.shopier_commission_value || 0),
