  ...
  return (
    <AdminLayout>
                  ...
                  <tr key={u.id} className="border-t border-gray-50 hover:bg-gray-50/50">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <div className="font-bold text-gray-800 flex items-center gap-1">
                            {u.username}
                            {u.is_admin === 1 && <span className="text-[10px] bg-violet-100 text-violet-600 font-bold px-1.5 py-0.5 rounded-full">Admin</span>}
                          </div>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <button onClick={() => handleToggleAdmin(u)} title={u.is_admin ? 'Admin Kaldır' : 'Admin Yap'} className="p-1.5 rounded-lg hover:bg-violet-50 text-gray-500 hover:text-violet-600"><ShieldCheck size={15} /></button>
                        <button onClick={() => { setSelectedUser(u); setBanReason(u.ban_reason || ''); setModalType('ban'); }} title={u.is_banned ? 'Ban Kaldır' : 'Banla'} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-500 hover:text-red-600"><Ban size={15} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {modalType === 'ban' && selectedUser && (
        <Modal title={selectedUser.is_banned ? 'Ban Kaldır' : `${selectedUser.username} Kullanıcısını Banla`} onClose={() => setModalType('')}>
          ...
          <button onClick={handleBan} disabled={saving} className={`w-full py-2.5 rounded-xl font-bold text-sm transition-all disabled:opacity-50 text-white ${selectedUser.is_banned ? 'bg-emerald-600 hover:bg-emerald-500' : 'bg-red-600 hover:bg-red-500'}`}>
            {saving ? 'Kaydediliyor...' : (selectedUser.is_banned ? 'Banı Kaldır' : 'Banla')}
          </button>
        </Modal>
      )}
