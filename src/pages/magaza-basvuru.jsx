...
export default function StoreApplicationPage() {
  ...
  return (
    <div className="mx-auto max-w-screen-2xl space-y-8">
      ...
      {!isVerified ? (
        <section className="mb-fade-up relative overflow-hidden rounded-3xl border border-slate-100 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6" style={{ animationDelay: '.15s' }}>
          ...
          <div className="mb-5">
            <div className="mb-1.5 flex items-center justify-between text-[11px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">
              <span>İlerleme</span>
              <span className="text-slate-900 dark:text-white">{criteriaProgress}%</span>
            </div>
            <div className="mb-shimmer-wrap h-2.5 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
              <div
            ...
            {criteria.items?.map((item) => (
              <div
                ...
                className={`flex gap-3 rounded-2xl border p-3.5 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md ${
                  item.passed
                    ? 'border-emerald-100 bg-emerald-50/50 dark:border-emerald-900/40 dark:bg-emerald-950/30'
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs font-semibold leading-5 text-slate-500 dark:text-slate-400">{item.description}</p>
                  {item.required !== undefined ? (
                    <div className={`mt-1 text-[11px] font-black ${
                      item.passed ? 'text-emerald-600 dark:text-emerald-300' : 'text-slate-400 dark:text-slate-500'
                    }`}>
