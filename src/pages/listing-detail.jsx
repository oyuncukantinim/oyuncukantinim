    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="card p-6 sm:p-8 flex flex-col lg:flex-row gap-8">
        ...
        <div className="w-full lg:w-1/2 space-y-4">
          <div className="w-full aspect-video bg-dark-600 rounded-2xl overflow-hidden">
            {images.length > 0 ? (
            ...
            <div className="flex gap-3 overflow-x-auto pb-2">
              {images.map((img, idx) => (
                <button
                  ...
                  className={`flex-shrink-0 w-20 aspect-video rounded-xl overflow-hidden border-2 transition-all ${activeImg === idx ? 'border-neon-purple opacity-100' : 'border-transparent opacity-40 hover:opacity-80'}`}
                >
        ...
        <div className="w-full lg:w-1/2 flex flex-col">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-gray-500 text-sm font-bold flex items-center gap-1">
              <Gamepad2 size={14} /> {listing.game_name}
            </span>
          </div>

          <h1 className="text-3xl font-extrabold text-white mb-4 leading-tight">{listing.title}</h1>

          {/* Seller */}
          <div className="flex items-center gap-4 mb-6 bg-dark-600/50 p-4 rounded-2xl border border-white/5">
            <div className="w-12 h-12 bg-dark-700 rounded-xl flex items-center justify-center text-2xl">
              {listing.avatar || '👤'}
            </div>
            <div className="flex-1">
              <div className="text-sm font-bold text-gray-300">Satici: {listing.seller}</div>
              <div className="flex items-center text-xs font-bold text-yellow-500 mt-1">
                <Star size={12} className="fill-current mr-1" /> {listing.rating || 5.0} / 5.0
              </div>
            </div>
            {user && !isSeller && (
              <Link
          ...
          <div className="mb-8 flex-1">
            <h3 className="font-bold text-gray-300 mb-3">Ilan Aciklamasi</h3>
            <div className="text-gray-400 text-sm leading-relaxed bg-dark-600/50 p-5 rounded-2xl border border-white/5 whitespace-pre-wrap min-h-[100px]">
          ...
          <div className="pt-6 border-t border-white/5 flex items-center justify-between mt-auto">
            ...
            {isSeller ? (
              <span className="badge-purple py-2 px-4 text-sm">Kendi ilanin</span>
            ) : (
              <button onClick={handleBuy} className="btn-primary flex items-center gap-2 text-lg py-4 px-8">
