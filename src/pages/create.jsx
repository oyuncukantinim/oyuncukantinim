                ...
                {[
                  ...
                  return (
                    <div key={type} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                      {/* Üst: Başlık + Açıklama */}
                      <div className="mb-4 flex flex-col gap-1 sm:flex-row sm:items-start sm:gap-4">
                        <div className={`inline-flex shrink-0 rounded-full border px-3 py-1 text-xs font-bold ${meta.buttonClass}`}>
                          {meta.label}
                        </div>
                        <p className="text-sm leading-6 text-slate-600">{meta.description}</p>
                      </div>
                      {/* Alt: Görsel (sol) + Seçenekler (sağ) */}
                      <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
                        <div className={`shrink-0 overflow-hidden rounded-2xl border ${meta.accentClass} w-full sm:w-40`}>
                          {dopingImage ? (
                            <img src={dopingImage} alt={meta.label} className="h-48 w-full object-cover sm:h-full" />
                          ) : (
                            <div className="flex h-48 items-center justify-center bg-slate-50 text-sm font-black text-slate-400 sm:h-full sm:min-h-[200px]">
                              {meta.label}
                            </div>
                          )}
                        </div>
                        <div className="flex-1">
                          <div className="grid grid-cols-3 gap-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6">
                            <button
                              ...
                              className={`rounded-xl border p-2.5 text-left transition-all ${!selectedHours ? 'border-slate-500 bg-slate-100 shadow-sm shadow-slate-200' : 'border-slate-200 bg-slate-50/60 hover:border-slate-300 hover:bg-white'}`}
                            >
                              <div className="text-center">
                                <div className="text-lg leading-none">🛇</div>
                                <div className="mt-1.5 text-[10px] font-bold text-emerald-600">0.00₺</div>
                                {!selectedHours ? <span className="mt-1 inline-block rounded-full bg-slate-700 px-1.5 py-0.5 text-[9px] font-bold text-white">Seçili</span> : null}
                              </div>
                            </button>
                            ...
                            {options.map((option) => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`rounded-xl border p-2.5 text-left transition-all ${selected ? 'border-violet-500 bg-violet-50 shadow-sm shadow-violet-100' : 'border-slate-200 bg-slate-50/60 hover:border-violet-200 hover:bg-white'}`}
                                >
                                  <div className="text-center">
                                    <div className="text-xs font-black text-slate-900">{formatDopingDuration(option.hours)}</div>
                                    <div className="mt-1 text-[10px] font-semibold text-slate-500">{Number(option.price).toFixed(2)}₺</div>
                                    {selected ? <span className="mt-1 inline-block rounded-full bg-violet-600 px-1.5 py-0.5 text-[9px] font-bold text-white">Seçili</span> : null}
                                    {insufficient ? (
                                      <div className="mt-1 text-[9px] font-bold text-red-500">Yetersiz</div>
                                    ) : null}
                                  </div>
                                </button>
                              );
