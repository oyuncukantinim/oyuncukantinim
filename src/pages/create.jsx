                        ...
                        <div className="flex-1">
                          <div className="grid grid-cols-4 gap-1.5 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8">
                            <button
                              ...
                              className={`rounded-lg border px-1.5 py-2 transition-all ${!selectedHours ? 'border-slate-500 bg-slate-100 shadow-sm shadow-slate-200' : 'border-slate-200 bg-slate-50/60 hover:border-slate-300 hover:bg-white'}`}
                            >
                              <div className="text-center">
                                <div className="text-lg leading-none">🛇</div>
                                <div className="mt-1 text-sm font-bold text-emerald-600">0₺</div>
                                {!selectedHours ? <span className="mt-1 inline-block rounded-full bg-slate-700 px-1.5 py-0.5 text-[11px] font-bold text-white">Seçili</span> : null}
                              </div>
                            </button>
                            ...
                            {options.map((option) => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`rounded-lg border px-1.5 py-2 transition-all ${selected ? 'border-violet-500 bg-violet-50 shadow-sm shadow-violet-100' : 'border-slate-200 bg-slate-50/60 hover:border-violet-200 hover:bg-white'}`}
                                >
                                  <div className="text-center">
                                    <div className="text-base font-black text-slate-900">{formatDopingDuration(option.hours)}</div>
                                    <div className="mt-0.5 text-sm font-semibold text-slate-500">{Number(option.price).toFixed(2)}₺</div>
                                    {selected ? <span className="mt-1 inline-block rounded-full bg-violet-600 px-1.5 py-0.5 text-[11px] font-bold text-white">Seçili</span> : null}
                                    {insufficient ? (
                                      <div className="mt-1 text-[11px] font-bold text-red-500">Yetersiz</div>
                                    ) : null}
                                  </div>
                                </button>
                              );
