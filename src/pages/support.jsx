    ...
    <div className="mx-auto max-w-[1760px] space-y-5">
      ...
      <section className="rounded-[26px] border border-slate-200 bg-white p-5 shadow-sm">
        <div className="mb-4 flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between"><div className="flex items-center gap-2"><div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-violet-100 text-violet-700"><Plus size={18} /></div><div><h2 className="text-lg font-black text-slate-900">Yeni Destek Talebi</h2><p className="text-xs text-slate-500">{introText}</p></div></div><div className="flex items-center gap-3 xl:justify-end">
          ...
          {wizardStep < 2 ? <button type="button" disabled={!canAdvance} onClick={() => setWizardStep((prev) => prev + 1)} className="rounded-2xl bg-violet-600 px-5 py-3 text-sm font-black text-white disabled:opacity-40">{wizardStep === 1 ? `Devam Et (${form.selected_listing_ids.length} ilan seçtin)` : 'Devam Et'}</button> : <button type="submit" form="support-ticket-form" disabled={creating || !canAdvance} className="rounded-2xl bg-violet-600 px-5 py-3 text-sm font-black text-white disabled:opacity-40">{creating ? 'Oluşturuluyor...' : 'Destek Talebi Oluştur'}</button>}
        </div></div>
        ...
        <form id="support-ticket-form" onSubmit={submitTicket} className="space-y-5">
          {wizardStep === 0 ? (
            ...
            <div className="space-y-4">
              <div className="space-y-4">
                ...
                <div className="rounded-[22px] border border-slate-200 bg-slate-50 p-3">
                  ...
                  {activeListingPool.length ? activeListingPool.map((item) => <ListingPickerRow key={`${form.related_scope}-${item.id}-${item.order_id || 'listing'}`} item={{ ...item, item_image: getListingCoverImage(item, defaultListingImage) }} selected={form.selected_listing_ids.includes(Number(item.id))} onToggle={toggleListing} scope={form.related_scope} />) : <div className="px-2 py-6 text-sm text-slate-400">Bu alt kategoride seçilebilir ilan bulunamadı.</div>}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex min-h-[640px] flex-col">
              <div className="border-b border-slate-100 pb-4">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div><div className="mb-2 flex flex-wrap items-center gap-2"><span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-extrabold uppercase tracking-[0.16em] text-slate-500">{selectedTicket.ticket_no}</span><StatusBadge value={selectedTicket.status} compact /></div><h2 className="text-lg font-black tracking-tight text-slate-950">{selectedTicket.subject}</h2><div className="mt-2 flex flex-wrap items-center gap-3 text-sm text-slate-500"><span className="inline-flex items-center gap-1.5"><Clock3 size={14} />{fmtDate(selectedTicket.created_at)}</span></div></div>
                  {selectedTicket.status !== 'closed' ? <button onClick={closeTicket} disabled={closing} className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-black text-rose-700 disabled:opacity-40"><XCircle size={14} />{closing ? 'Kapatılıyor...' : 'Talebi Kapat'}</button> : null}
                </div>
