  ...
  return (
    <div className="space-y-8">
      <div className="card p-8 flex flex-col md:flex-row items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black mb-2 text-gray-800">Oyuncu Pazari</h1>
          <p className="text-gray-500">Diger oyunculardan hesap, esya ve boost hizmeti satin al.</p>
        </div>
        {user && (
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Ilan ara..." className="input-field pl-10" />
          </div>
          <div className="flex items-center gap-2">
            <SlidersHorizontal size={16} className="text-gray-400" />
            <select value={sort} onChange={e => setSort(e.target.value)} className="input-field w-auto min-w-[140px]">
      ...
      {loading ? (
        ...
        <div className="text-center text-gray-400 py-16">
          ...
          {user && <Link to="/create" className="text-neon-purple font-bold hover:underline">Ilk ilani sen ekle!</Link>}
        </div>
      ) : (
